...
```bash
../scripts/create-matlab-vm.sh --name matlab-linux-01 --os linux
../scripts/create-matlab-vm.sh --name matlab-windows-01 --os windows
```

The script supplies the managed image's full Azure resource ID to
`az vm create`. Use `--image-id` to deploy a managed image whose name or
resource group differs from the template defaults.
